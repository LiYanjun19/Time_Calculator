...
def input_init(clock_in, lunchbreak_start, lunchbreak_end, clock_out):
    ...
    if clock_in == 0:
        mode = 4
        clockin = HourMin(0,0)
    elif clock_in < 2400 and clock_in > 0:
    ...
    ## lunch break start
    if lunchbreak_start == 0 and (mode == 1 or mode == 4) :
        lb_start = HourMin(0,0)
        
    elif lunchbreak_start < 2400 and lunchbreak_start > clock_in:
    ...
    ## lunch break end
    if lunchbreak_end == 0 and (mode == 1 or mode == 2 or mode == 4):
        lb_end = HourMin(0,0) 
    elif lunchbreak_end < 2400 and lunchbreak_end > lunchbreak_start:
    ...
    ## clock out 
    if clock_out == 0 and (mode == 3 or mode == 2 or mode == 1):
        clockout = HourMin(0,0)
    elif clock_out > 0 and mode == 4:
        if clock_out < 2400:
    ...
    return clockin, lb_start, lb_end, clockout, mode
